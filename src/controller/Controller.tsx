...
) 
{
  ...
  messageReceiver.setTaskSwitchRequestListener((sendingWindow: MessageEventSource, request: RequestType, requestDetails?: TaskRequestDetails) => {
    ...
    const currentTask = taskSequencer.currentTask;    
      ...
      case 'blocked': 
        ...
        if(request === "nextTask"){
            ...
            messageReceiver.setGetScoringResultReturnListener((sendingWindow: MessageEventSource, result: string) => {
                window.postMessage(
                  JSON.stringify({eventType: "progress_endOfSequence"}),
                  "*"
                );
                messageReceiver.setGetScoringResultReturnListener((sendingWindow: MessageEventSource, result: string) => {});
            });
  ...
  messageReceiver.setPlayerReadyListener((sendingWindow) => {
    ...
    downloadAssessmentConfig()
      .then((configuration) => {
        ...
        let _sc = extractScalingConfigurationFromQuery();
        taskSequencer.initialize(configuration, playerCatalog);
        playerCatalog.doToAll((targetWindow: MessageEventSource) => setScalingConfiguration(targetWindow, _sc)); 
        loadItemsAndStartFirstTask(configuration, controllerConfig.mathJaxCdnUrl, playerCatalog, itemCatalog, taskSequencer);
      })
      .catch((error) => {
        console.warn(`Could not initialize assessment properly: ${error.message}`);
  ...
  messageReceiver.setTraceLogListener((sendingWindow, data) => {
    // getScoringResult(sendingWindow);
    sendMessageToTaskPlayer(sendingWindow, { eventType: 'getScoringResult', requestId: `${taskSequencer.currentTask?.item}.${taskSequencer.currentTask?.task}`});
  });
