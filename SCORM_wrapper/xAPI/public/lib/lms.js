;(function(root) {
  ...
  function IbSendTracelog(data){
    commitData({
      description: "event/"+data?.tracecount,
      id: "event/"+data?.tracecount,
      name: "event/"+data?.tracecount,
      result: {
        response: JSON.stringify(data?.tracelog)
      },
      type: INTERACTION,
  ...
  function IbReportAnswer(data){
    commitData({
      description: data.itemId + '/' + data.id,
      id: data.itemId + '/' + data.id,
      name: data.itemId + '/' + data.id,
      result: {
        // success: data.isCorrect,
  ...
  function initialize(_entryCb) {
    
    if(initialized)
      return;
    
    if(typeof _entryCb == "function")
      entryCb = _entryCb;
  ...
  function lms() {
    ...
    return {
      ...
      IbReportAnswer: IbReportAnswer,
      IbSendTracelog: IbSendTracelog,
      utils: {
        assign: assign,
        debounce: debounce
      }
